...
from lib.models.layers.utils import get_distribution_target


class Attention(nn.Module):
    def __init__(self, dim, num_heads, qkv_bias, attn_drop=0., proj_drop=0., divide=False):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x, search_feat_len, threshold=0., ratio=0.):
        B, N, C = x.shape
        decision = None
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, num_patches=197, divide=False):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, divide=divide)
    ...
    def forward(self, x, search_feat_len, threshold=0.):
        bs, token, dim = x.shape

        x = x + self.drop_path(self.attn(self.norm1(x), search_feat_len, threshold=threshold))
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                num_patches=self.patch_embed.num_patches+1, divide=bool(i)
                )
...
def _create_vision_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
    ...
    if pretrained:
        path = "/root/MaskGRM/pretrained_models/mae_pretrain_vit_base.pth"
...
def vit_base_patch16_224_base(pretrained=False, **kwargs):
    ...
    """
    args = {'act_mode': 4, 'gate_scale': 10.0, 'gate_center': 30.0, 'distr_prior_alpha': 0.01}
    args = Namespace(**args)
    model_kwargs = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, args=args, **kwargs)
