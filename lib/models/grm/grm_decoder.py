...
class GRM(nn.Module):
    ...
    def __init__(self, transformer, box_head, decoder, head_type='CORNER'):
        """
    ...
    def forward(self, template, search, threshold=0., softmax=True, gt_bboxes=None):
        x = self.backbone(z=template, x=search, search_feat_len=self.feat_len_s, threshold=threshold)

        # Forward head
        feat_last = x
        if isinstance(x, list):
            feat_last = x[-1]
        out = self.forward_head(feat_last, softmax=softmax)

        if self.decoder:
            loss = self.decoder(x, template, search)
            out['renew_loss'] = loss
...
def build_decoder_grm(cfg, training=True):
