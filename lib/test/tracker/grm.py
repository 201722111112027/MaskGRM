...
from lib.models.grm.grm_decoder import build_decoder_grm
...
class GRM(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        network = build_decoder_grm(params.cfg, training=False)
        missing_keys, unexpected_keys = network.load_state_dict(torch.load(self.params.checkpoint,
                                                                           map_location='cpu')['net'], strict=False)
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.no_grad():
            ...
            # Run the transformer
            out_dict = self.network.forward(template=self.z_dict1.tensors, search=x_dict.tensors, threshold=self.threshold)
