...
# network related
from lib.models.grm import build_grm, build_decoder_grm
...
def run(settings):
    ...
    net = build_grm(cfg, cfg)
    net_teacher = build_decoder_grm(cfg, False)
    ...
    if settings.script_name == 'grm':
        ...
        loss_weight = {'ciou': cfg.TRAIN.IOU_WEIGHT, 'l1': cfg.TRAIN.L1_WEIGHT,  'corner': cfg.TRAIN.CORNER_WEIGHT}
        actor = GRMActor(net=net, net_teacher=net_teacher, objective=objective, loss_weight=loss_weight,
                         settings=settings, run_score_head=False, z_size_teacher=cfg_teacher.DATA.TEMPLATE.SIZE,
                         x_size_teacher=cfg_teacher.DATA.SEARCH.SIZE, feat_sz=cfg.MODEL.FEAT_SZ)
    else:
